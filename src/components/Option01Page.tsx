...
/* ── App menu data ──
   ...
   this prototype. "Option 01" is marked active here since this file *is*
   that page. */
function buildAppMenuGroups(onNavigate?: (page: Page) => void): AppMenuGroup[] {
  return [
    {
      items: [
        { label: "Option 01", active: true, onClick: () => onNavigate?.("option-01") },
...
/* ── Option01Page ──
   ...
   and its Option02Page/Option03Page/Option04Page duplicates) loads its
   content directly. */
            ...
            <PopoverPrimitive.Portal>
              ...
              >
                <AppMenu
                  ...
                  header={isCompactHeader ? "Agent Nav Test Option 01" : undefined}
                />
              </PopoverPrimitive.Content>
            </PopoverPrimitive.Portal>
        ...
        <LeftNav
          ...
          pinnedHeader={
            <>
              ...
              {/* Static "Home" row — per instruction, no more hover flyout;
                  this is a plain nav-anchor button back to the Desk
                  dashboard, always showing a fixed "Home" label/icon
                  regardless of which Desk tab is active (previously tracked
                  `activeDeskTab` via DESK_TAB_ICONS/DESK_TAB_LABELS). Icon
                  is lucide's `Home` (house shape), reused from the app-menu
                  entry above. */}
              <Tooltip content="Home" placement="right" disabled={navOpen}>
                <button
                  type="button"
                  aria-label="Home"
                ...
                >
                  <Home className="h-4 w-4 flex-shrink-0" strokeWidth={1.5} aria-hidden="true" />
                  {navOpen && <span className="flex-1 truncate text-left">Home</span>}
                </button>
              </Tooltip>
                ...
                <>
                  ...
                  {showPageHeader && (
                    <TabList overflowMenu className="px-6 bg-lyra-bg-surface-base shrink-0">
                      {/* Dashboard has no same-named nav item in 02/03 to
                          match (their equivalent is "Home", a different
                          label) — its own icon, not borrowed from either.
                          Every other tab below reuses the exact icon its
                          same-named nav item uses in 02/03 (see
                          PINNABLE_NAV_META/the rail's railItemMeta there). */}
                      <Tab
                        active={activeDeskTab === "dashboard"}
                        onClick={() => setActiveDeskTab("dashboard")}
                        icon={<LayoutDashboard className="h-4 w-4" strokeWidth={1.5} />}
                      >
                        Dashboard
                      </Tab>
                      <Tab
                        active={activeDeskTab === "customers"}
                        onClick={() => setActiveDeskTab("customers")}
                        icon={<Users className="h-4 w-4" strokeWidth={1.5} />}
                      >
                        Customers
                      </Tab>
                      <Tab
                        active={activeDeskTab === "accounts"}
                        onClick={() => setActiveDeskTab("accounts")}
                        icon={<Landmark className="h-4 w-4" strokeWidth={1.5} />}
                      >
                        Accounts
                      </Tab>
                      <Tab
                        active={activeDeskTab === "tickets"}
                        onClick={() => setActiveDeskTab("tickets")}
                        icon={<Ticket className="h-4 w-4" strokeWidth={1.5} />}
                      >
                        Tickets
                      </Tab>
                      <Tab
                        active={activeDeskTab === "wem"}
                        onClick={() => setActiveDeskTab("wem")}
                        icon={<CalendarClock className="h-4 w-4" strokeWidth={1.5} />}
                      >
                        WEM
                      </Tab>
                      <Tab
                        active={activeDeskTab === "directory"}
                        onClick={() => setActiveDeskTab("directory")}
                        icon={<BookUser className="h-4 w-4" strokeWidth={1.5} />}
                      >
                        Directory
                      </Tab>
                      <Tab
                        active={activeDeskTab === "settings"}
                      ...
                      <Tab
                        active={activeDeskTab === "search"}
                        onClick={() => setActiveDeskTab("search")}
